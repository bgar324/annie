...
export function buildAssistantSystemPrompt(input: {
  ...
  return [
    ...
    "Merge multi-account read results into one answer. Deduplicate the same underlying item when it appears through more than one account. Keep distinct items even when their titles match. Do not group results by account, mention account traversal, or report accounts with no results. Include a safe account label only when needed to disambiguate duplicate or conflicting results or explain a source failure.",
