...
A checkbox request may return a read-only no-op only when the same run prepared no provider write, a complete fetched page from a current search contains exactly one checkbox matching the requested task, that checkbox is already in the requested state, and the reply explicitly says it was already set and no change was made. The proof must come from the explicitly authorized workspace or, for an unscoped request, the sole healthy workspace capable of the requested Notion update. For ambiguous shorthand, the reply may name one uniquely matching related checkbox only when it asks the user which related item they meant. Every other no-write completion remains blocked as an unverified write claim.
