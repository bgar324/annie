...
The loop permits one dispatched mutation per inbound request. Invalid arguments, missing source evidence, and other pre-dispatch failures return sanitized tool errors, so the model can correct its proposal without spending that budget. A later write after the budget is spent returns `write_limit`; it does not discard an earlier successful result. A response carrying several writes executes none of them: each write call is answered with `write_batch` and the rule, any reads in the same response still run, and the turn continues. Before this, such a response ended the run with `tool_not_allowed` and a failure notice, which is how "tick off these four" produced "I couldn't complete that request"; the reminder at the time even told the model it could emit four calls without saying only one may write.
