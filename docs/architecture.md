...
An explicit write action can refer to a target established by context, but a bare acknowledgement does not authorize an old request. The runtime does not use an English command grammar or a final-prose validator.
