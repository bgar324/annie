...
Before loading history, memory, or account data, an inbound run classifies only its current raw message. This tool-free JSON request uses low reasoning and a 1536-token output cap. Its immutable, lease-fenced `agent_runs.request_scope` decides what needs authorization: Notion writes or one provider's connection link. Everything else, `conversation` and `read` alike, receives the read tools, because a read mutates nothing and the classifier's only job is to keep history from granting a write; a classifier miss on a two-word message therefore costs at most a spurious lookup. The contextual loop cannot widen those permissions. Classification shares the original run deadline and consumes one counted model request; ordinary agent and memory requests keep high reasoning. Asking whether Annie has access to, can see, or can find a named item is `read`. A reminder, alarm, notification, email, text, or calendar request is not a Notion write; the model may offer a list entry, and the user's answer completes it through the follow-up path. When the classifier spends its whole output budget or returns something unparseable, the run does not fail: `read` is the floor, and the trace records `request_scope.fallback` with the reason. Earlier designs gave `conversation` no tools at all, and a tool-less run either returned an empty message (a failure notice) or, once told to answer in text, composed the user's own request back to them and asked for it "as one complete message"; both were the same defect, a scope with nothing to act with, and both are gone. Tool argument validation names each violation (`/queries/0 must NOT have additional properties 'account'`) so the model corrects its next call; an opaque "does not match the schema" cost a round in one of three calendar reads.
...
An explicit write action can refer to a target established by context, but a bare acknowledgement does not authorize an old request. The exceptions are bounded and mechanical, and both come from what Annie herself last delivered. `ConversationHistoryStore.precedingContext` looks at the egress for the previous accepted message's run, only if it is in state `delivered`, its delivery confirmation precedes the user's send time of the current message by at most 30 minutes, and nothing else went to the user in between; connection links, undelivered or delivery-unknown messages, messages confirmed only after the answer was sent, and older messages contribute nothing, and delivery confirmation lags the device by a poll, so the gate errs toward supplying nothing. A `reply` gives the classifier Annie's own words, so a direct answer to a question she just asked ("Logit notes" after "tell me the page name and i'll make it", "yes" after "want me to check it off?") completes that offered action. A `failure` notice gives the classifier the user's own failed request and the scope that run already earned, so "try again" repeats that request under that scope; the notice text itself is never shown, and a failure whose run never reached classification (a media-only message) supplies nothing. Before this, "Try again" after a failure notice was plain conversation: the model, holding the history but no tools, composed the request back to the user and asked them to resend it. Earlier user messages never otherwise reach the classifier. The trace records `request_scope.preceding_reply` as `reply`, `failure`, or `none`. The runtime does not use an English command grammar or a final-prose validator.
