...
An ambiguous shorthand request may instead return a read-only target clarification when the complete fetched page contains multiple plausible related labels, the reply names exactly one uniquely occurring related checkbox already in the requested state, explicitly says no change was made, and asks which related item the user intended. Both branches require proof from the explicitly authorized workspace or, for an unscoped request, the sole healthy workspace capable of the requested Notion update. Every other no-write completion remains blocked as an unverified write claim.
